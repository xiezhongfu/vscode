...
let MenuEntryActionViewItem = class MenuEntryActionViewItem extends ActionViewItem {
    ...
    getTooltip() {
        ...
        let title = keybindingLabel
            ? localize(1771, "{0} ({1})", tooltip, keybindingLabel)
            : tooltip;
        if (!this._wantsAltCommand && this._menuItemAction.alt?.enabled) {
            ...
            const altTitleSection = altKeybindingLabel
                ? localize(1772, "{0} ({1})", altTooltip, altKeybindingLabel)
                : altTooltip;
            title = localize(1773, "{0}\n[{1}] {2}", title, UILabelProvider.modifierLabels[OS].altKey, altTitleSection);
        }
        return title;
    }
    updateClass() {
    ...
    updateLabel() {
        ...
        if (this.label) {
            ...
            if (this._options?.conversational) {
                this.label.textContent = localize(1774, '{1} to {0}', this._action.label, kb2);
            }
            else {
                this.label.textContent = localize(1775, '{0} ({1})', this._action.label, kb2);
            }
        }
    }
