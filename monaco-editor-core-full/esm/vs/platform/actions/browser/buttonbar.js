...
let WorkbenchButtonBar = class WorkbenchButtonBar extends ButtonBar {
    ...
    update(actions, secondary) {
        ...
        for (let i = 0; i < actions.length; i++) {
            ...
            if (kb) {
                tooltip = localize(1767, "{0} ({1})", actionOrSubmenu.tooltip || actionOrSubmenu.label, kb.getLabel());
            }
            else {
        ...
        if (secondary.length > 0) {
            const btn = this.addButton({
                secondary: true,
                ariaLabel: localize(1768, "More Actions")
            });
            btn.icon = Codicon.dropDownButton;
            btn.element.classList.add('default-colors', 'monaco-text-button');
            btn.enabled = true;
            this._updateStore.add(this._hoverService.setupManagedHover(hoverDelegate, btn.element, localize(1769, "More Actions")));
            this._updateStore.add(btn.onDidClick(async () => {
