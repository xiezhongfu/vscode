...
export function ensureFileSystemProviderError(error) {
    if (!error) {
        return createFileSystemProviderError(localize(1865, "Unknown Error"), FileSystemProviderErrorCode.Unknown); // https://github.com/microsoft/vscode/issues/72798
    }
    return error;
}
...
export class ByteSize {
    ...
    static formatSize(size) {
        ...
        if (size < ByteSize.KB) {
            return localize(1866, "{0}B", size.toFixed(0));
        }
        if (size < ByteSize.MB) {
            return localize(1867, "{0}KB", (size / ByteSize.KB).toFixed(2));
        }
        if (size < ByteSize.GB) {
            return localize(1868, "{0}MB", (size / ByteSize.MB).toFixed(2));
        }
        if (size < ByteSize.TB) {
            return localize(1869, "{0}GB", (size / ByteSize.GB).toFixed(2));
        }
        return localize(1870, "{0}TB", (size / ByteSize.TB).toFixed(2));
    }
}
