...
export class HTMLFileSystemProvider extends Disposable {
    ...
    async rename(from, to, opts) {
        try {
            ...
            // File API does not support any real rename otherwise
                ...
                throw this.createFileSystemProviderError(from, localize(1857, "Rename is only supported for files."), FileSystemProviderErrorCode.Unavailable);
            }
        }
        catch (error) {
            throw this.toFileSystemProviderError(error);
    ...
    toFileSystemProviderError(error) {
        ...
        if (error.name === 'NotAllowedError') {
            error = new Error(localize(1858, "Insufficient permissions. Please retry and allow the operation."));
