...
const errorEmptyString = localize(1802, "Empty context key expression");
const hintEmptyString = localize(1803, "Did you forget to write an expression? You can also put 'false' or 'true' to always evaluate to false or true, respectively.");
const errorNoInAfterNot = localize(1804, "'in' after 'not'.");
const errorClosingParenthesis = localize(1805, "closing parenthesis ')'");
const errorUnexpectedToken = localize(1806, "Unexpected token");
const hintUnexpectedToken = localize(1807, "Did you forget to put && or || before the token?");
const errorUnexpectedEOF = localize(1808, "Unexpected end of expression");
const hintUnexpectedEOF = localize(1809, "Did you forget to put a context key?");
/**
...
export class Parser {
    ...
    _errExpectedButGot(expected, got, additionalInfo) {
        const message = localize(1810, "Expected: {0}\nReceived: '{1}'.", expected, Scanner.getLexeme(got));
        ...
        if (parser.lexingErrors.length > 0) {
            return parser.lexingErrors.map((se) => ({
                errorMessage: se.additionalInfo ?
                    localize(1811, "Unexpected token. Hint: {0}", se.additionalInfo) :
                    localize(1812, "Unexpected token."),
                offset: se.offset,
