            ...
            case DialogKind.Prompt: {
                ...
                if (promptDialog.cancelButton) {
                    if (promptDialog.cancelButton === true) {
                        buttons.push(localize(1830, "Cancel"));
                    }
                        ...
                        if (promptDialog.cancelButton.label) {
                            ...
                            buttons.push(localize(1831, "Cancel"));
                        }
                    }
                }
                if (buttons.length === 0) {
                    buttons.push(localize(1832, "&&OK"));
                }
                break;
            }
...
export function getFileNamesMessage(fileNamesOrResources) {
    ...
    if (fileNamesOrResources.length > MAX_CONFIRM_FILES) {
        if (fileNamesOrResources.length - MAX_CONFIRM_FILES === 1) {
            message.push(localize(1835, "...1 additional file not shown"));
        }
        else {
            message.push(localize(1836, "...{0} additional files not shown", fileNamesOrResources.length - MAX_CONFIRM_FILES));
        }
    }
    message.push('');
    return message.join('\n');
