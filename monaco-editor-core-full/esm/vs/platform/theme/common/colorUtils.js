...
class ColorRegistry extends Disposable {
    ...
    registerColor(id, defaults, description, needsTransparency = false, deprecationMessage) {
        ...
        if (needsTransparency) {
            ...
            propertySchema.patternErrorMessage = nls.localize(2210, 'This color must be transparent or it will obscure content');
        }
        this.colorSchema.properties[id] = {
            description,
            oneOf: [
                propertySchema,
                { type: 'string', const: DEFAULT_COLOR_CONFIG_VALUE, description: nls.localize(2211, 'Use the default color.') }
            ]
        };
