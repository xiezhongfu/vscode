...
class FindWidget extends Disposable {
    ...
    constructor(container, tree, contextViewProvider, placeholder, toggleContributions = [], options) {
        ...
        this.findInput = this._register(new FindInput(this.elements.findInput, contextViewProvider, {
            label: localize(31, "Type to search"),
            placeholder,
        ...
        const closeAction = this._register(new Action('close', localize(32, "Close"), 'codicon codicon-close', true, () => this.dispose()));
...
export class AbstractFindController {
    ...
    constructor(tree, filter, contextViewProvider, options = {}) {
        ...
        this._placeholder = options.placeholder ?? localize(33, "Type to search");
    }
    isOpened() {
        return !!this.widget;
    }
    ...
    renderMessage(showNotFound, warningMessage) {
        if (showNotFound) {
            if (this.tree.options.showNotFoundMessage ?? true) {
                this.widget?.showMessage({ type: 2 /* MessageType.WARNING */, content: warningMessage ?? localize(34, "No results found.") });
            }
            else {
...
export class FindController extends AbstractFindController {
    ...
    set mode(mode) {
        ...
        this.placeholder = isFilterMode ? localize(37, "Type to filter") : localize(38, "Type to search");
