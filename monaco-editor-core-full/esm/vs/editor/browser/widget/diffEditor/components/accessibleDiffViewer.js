...
const accessibleDiffViewerInsertIcon = registerIcon('diff-review-insert', Codicon.add, localize(178, 'Icon for \'Insert\' in accessible diff viewer.'));
const accessibleDiffViewerRemoveIcon = registerIcon('diff-review-remove', Codicon.remove, localize(179, 'Icon for \'Remove\' in accessible diff viewer.'));
const accessibleDiffViewerCloseIcon = registerIcon('diff-review-close', Codicon.close, localize(180, 'Icon for \'Close\' in accessible diff viewer.'));
...
let View = class View extends Disposable {
    ...
    _render(store) {
        ...
        container.setAttribute('aria-label', localize(182, 'Accessible Diff Viewer. Use arrow up and down to navigate.'));
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = (lines) => lines === 0 ? localize(183, "no lines changed")
                    : lines === 1 ? localize(184, "1 line changed")
                        : localize(185, "{0} lines changed", lines);
                ...
                header.setAttribute('aria-label', localize(186, "Difference {0} of {1}: original line {2}, {3}, modified line {4}, {5}", (diffIndex + 1), diffsLength, r.original.startLineNumber, originalChangedLinesCntAria, r.modified.startLineNumber, modifiedChangedLinesCntAria));
    ...
    _createRow(item, lineHeight, width, originalOptions, originalModel, originalModelOpts, modifiedOptions, modifiedModel, modifiedModelOpts) {
        ...
        if (lineContent.length === 0) {
            lineContent = localize(187, "blank");
        }
        let ariaLabel = '';
        switch (item.type) {
            case LineType.Unchanged:
                if (item.originalLineNumber === item.modifiedLineNumber) {
                    ariaLabel = localize(188, "{0} unchanged line {1}", lineContent, item.originalLineNumber);
                }
                else {
                    ariaLabel = localize(189, "{0} original line {1} modified line {2}", lineContent, item.originalLineNumber, item.modifiedLineNumber);
                }
                break;
            case LineType.Added:
                ariaLabel = localize(190, "+ {0} modified line {1}", lineContent, item.modifiedLineNumber);
                break;
            case LineType.Deleted:
                ariaLabel = localize(191, "- {0} original line {1}", lineContent, item.originalLineNumber);
                break;
        }
