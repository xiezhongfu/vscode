    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === 'invalid') {
                this.notificationService.warn(nls.localize(1400, 'Failed to open this link because it is not well-formed: {0}', link.url.toString()));
            }
            else if (messageOrError === 'missing') {
                this.notificationService.warn(nls.localize(1401, 'Failed to open this link because its target is missing.'));
            }
            else {
                onUnexpectedError(err);
            }
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip
        ? link.tooltip
        : executeCmd
            ? nls.localize(1402, 'Execute command')
            : nls.localize(1403, 'Follow link');
    const kb = useMetaKey
        ? platform.isMacintosh
            ? nls.localize(1404, "cmd + click")
            : nls.localize(1405, "ctrl + click")
        : platform.isMacintosh
            ? nls.localize(1406, "option + click")
            : nls.localize(1407, "alt + click");
    if (link.url) {
        let nativeLabel = '';
        if (/^command:/i.test(link.url.toString())) {
            ...
            if (match) {
                ...
                nativeLabel = nls.localize(1408, "Execute command {0}", commandId);
            }
        }
...
class OpenLinkAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(1409, "Open Link"),
