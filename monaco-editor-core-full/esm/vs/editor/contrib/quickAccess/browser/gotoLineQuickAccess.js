...
export class AbstractGotoLineQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
    ...
    provideWithoutTextEditor(picker) {
        const label = localize(1458, "Open a text editor first to go to a line.");
        picker.items = [{ label }];
    ...
    provideWithTextEditor(context, picker, token) {
        ...
        const toggle = new Toggle({
            title: localize(1459, "Use Zero-Based Offset"),
    ...
    parsePosition(editor, value) {
        ...
        if (!model) {
            return {
                label: localize(1460, "Open a text editor first to go to a line.")
            };
        }
        ...
        if (value.startsWith(':')) {
            ...
            if (isNaN(offset)) {
                ...
                return {
                    ...
                    label: this.useZeroBasedOffset ?
                        localize(1461, "Type a character position to go to (from 0 to {0}).", maxOffset - 1) :
                        localize(1462, "Type a character position to go to (from 1 to {0}).", maxOffset)
                };
            }
            else {
                ...
                return {
                    ...pos,
                    inOffsetMode: true,
                    label: localize(1463, "Press 'Enter' to go to line {0} at column {1}.", pos.lineNumber, pos.column)
                };
            }
        }
        else {
            ...
            if (parts.length < 1 || isNaN(lineNumber)) {
                return {
                    label: localize(1464, "Type a line number to go to (from 1 to {0}).", maxLine)
                };
            }
            ...
            if (parts.length < 2 || isNaN(column)) {
                return {
                    ...
                    label: parts.length < 2 ?
                        localize(1465, "Press 'Enter' to go to line {0} or enter : to add a column number.", lineNumber) :
                        localize(1466, "Press 'Enter' to go to line {0} or enter a column number (from 1 to {1}).", lineNumber, maxColumn)
                };
            }
            ...
            return {
                lineNumber,
                column,
                label: localize(1467, "Press 'Enter' to go to line {0} at column {1}.", lineNumber, column)
            };
        }
    }
}
