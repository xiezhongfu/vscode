...
class RenameSkeleton {
    ...
    async _provideRenameEdits(newName, i, rejects, token) {
        ...
        if (!result) {
            return this._provideRenameEdits(newName, i + 1, rejects.concat(nls.localize(1503, "No result.")), token);
        }
    ...
    async run() {
        ...
        try {
            ...
            if (e instanceof CancellationError) {
                ...
                if (typeof e === 'string' || isMarkdownString(e)) {
                    MessageController.get(this.editor)?.showMessage(e || nls.localize(1504, "An unknown error occurred while resolving rename location"), position);
                }
            }
            return undefined;
        }
        ...
        const renameOperation = raceCancellation(skeleton.provideRenameEdits(inputFieldResult.newName, cts2.token), cts2.token).then(async (renameResult) => {
            ...
            this._bulkEditService.apply(renameResult, {
                ...
                label: nls.localize(1505, "Renaming '{0}' to '{1}'", loc?.text, inputFieldResult.newName),
                code: 'undoredo.rename',
                quotableLabel: nls.localize(1506, "Renaming {0} to {1}", loc?.text, inputFieldResult.newName),
                respectAutoSaveConfig: true,
                reason: EditSources.rename(),
            }).then(result => {
                trace('edits applied');
                if (result.ariaSummary) {
                    alert(nls.localize(1507, "Successfully renamed '{0}' to '{1}'. Summary: {2}", loc.text, inputFieldResult.newName, result.ariaSummary));
                }
            }).catch(err => {
                trace(`error when applying edits ${JSON.stringify(err, null, '\t')}`);
                this._notificationService.error(nls.localize(1508, "Rename failed to apply edits"));
                this._logService.error(err);
            });
        }, err => {
            trace('error when providing rename edits', JSON.stringify(err, null, '\t'));
            this._notificationService.error(nls.localize(1509, "Rename failed to compute edits"));
            this._logService.error(err);
        }).finally(() => {
            cts2.dispose();
        });
...
export class RenameAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(1511, "Rename Symbol"),
...
registerAction2(class FocusNextRenameSuggestion extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...nls.localize2(1512, "Focus Next Rename Suggestion"),
            },
            precondition: CONTEXT_RENAME_INPUT_VISIBLE,
            keybinding: [
                {
...
registerAction2(class FocusPreviousRenameSuggestion extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...nls.localize2(1513, "Focus Previous Rename Suggestion"),
            },
            precondition: CONTEXT_RENAME_INPUT_VISIBLE,
            keybinding: [
                {
...
Registry.as(Extensions.Configuration).registerConfiguration({
    ...
    properties: {
        'editor.rename.enablePreview': {
            ...
            description: nls.localize(1510, "Enable/disable the ability to preview changes before renaming"),
            default: true,
            type: 'boolean'
        }
    }
