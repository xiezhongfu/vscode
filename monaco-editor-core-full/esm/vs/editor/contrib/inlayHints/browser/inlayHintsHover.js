    ...
    computeAsync(anchor, _lineDecorations, source, token) {
        ...
        return new AsyncIterableProducer(async (executor) => {
            ...
            if (isNonEmptyArray(part.item.hint.textEdits)) {
                executor.emitOne(new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(localize(1287, "Double-click to insert"))], false, 10001));
            }
            ...
            if (part.part.location || part.part.command) {
                ...
                const kb = useMetaKey
                    ? platform.isMacintosh
                        ? localize(1288, "cmd + click")
                        : localize(1289, "ctrl + click")
                    : platform.isMacintosh
                        ? localize(1290, "option + click")
                        : localize(1291, "alt + click");
                if (part.part.location && part.part.command) {
                    linkHint = new MarkdownString().appendText(localize(1292, 'Go to Definition ({0}), right click for more', kb));
                }
                else if (part.part.location) {
                    linkHint = new MarkdownString().appendText(localize(1293, 'Go to Definition ({0})', kb));
                }
                else if (part.part.command) {
                    linkHint = new MarkdownString(`[${localize(1294, "Execute Command")}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, { isTrusted: true });
                }
                if (linkHint) {
