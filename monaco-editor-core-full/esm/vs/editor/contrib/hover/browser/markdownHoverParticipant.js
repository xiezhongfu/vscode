...
const $ = dom.$;
const increaseHoverVerbosityIcon = registerIcon('hover-increase-verbosity', Codicon.add, nls.localize(1253, 'Icon for increaseing hover verbosity.'));
const decreaseHoverVerbosityIcon = registerIcon('hover-decrease-verbosity', Codicon.remove, nls.localize(1254, 'Icon for decreasing hover verbosity.'));
...
let MarkdownHoverParticipant = class MarkdownHoverParticipant {
    ...
    createLoadingMessage(anchor) {
        return new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(nls.localize(1255, "Loading..."))], false, 2000);
    }
    computeSync(anchor, lineDecorations) {
        ...
        if (stopRenderingLineAfter >= 0 && lineLength > stopRenderingLineAfter && anchor.range.startColumn >= stopRenderingLineAfter) {
            ...
            result.push(new MarkdownHover(this, anchor.range, [{
                    value: nls.localize(1256, "Rendering paused for long line for performance reasons. This can be configured via `editor.stopRenderingLineAfter`.")
                }], false, index++));
        }
        if (!stopRenderingMessage && typeof maxTokenizationLineLength === 'number' && lineLength >= maxTokenizationLineLength) {
            result.push(new MarkdownHover(this, anchor.range, [{
                    value: nls.localize(1257, "Tokenization is skipped for long lines for performance reasons. This can be configured via `editor.maxTokenizationLineLength`.")
                }], false, index++));
        }
...
export function labelForHoverVerbosityAction(keybindingService, action) {
    switch (action) {
        case HoverVerbosityAction.Increase: {
            ...
            return kb ?
                nls.localize(1258, "Increase Hover Verbosity ({0})", kb.getLabel()) :
                nls.localize(1259, "Increase Hover Verbosity");
        }
        case HoverVerbosityAction.Decrease: {
            const kb = keybindingService.lookupKeybinding(DECREASE_HOVER_VERBOSITY_ACTION_ID);
            return kb ?
                nls.localize(1260, "Decrease Hover Verbosity ({0})", kb.getLabel()) :
                nls.localize(1261, "Decrease Hover Verbosity");
        }
    }
}
