...
let RenderedContentHoverParts = class RenderedContentHoverParts extends Disposable {
    ...
    getAccessibleHoverContentAtIndex(index) {
        ...
        if (renderedPart.type === 'statusBar') {
            const statusBarDescription = [localize(1220, "This is a hover status bar.")];
            for (const action of renderedPart.actions) {
                const keybinding = action.actionKeybindingLabel;
                if (keybinding) {
                    statusBarDescription.push(localize(1221, "It has an action with label {0} and keybinding {1}.", action.actionLabel, keybinding));
                }
                else {
                    statusBarDescription.push(localize(1222, "It has an action with label {0}.", action.actionLabel));
                }
            }
